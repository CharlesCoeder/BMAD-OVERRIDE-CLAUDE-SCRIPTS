...
├── 6.6.0/                          # Scripts for BMAD 6.6.0 (project_name → core/config.yaml; tier-adaptive pipeline)
├── 6.8.0/                          # Scripts for BMAD 6.8.0 (.claude/skills/ path fix; baseline_commit review-scope guard)
├── 6.10.0/                         # Scripts for BMAD 6.10.0 (subagent standing authorization; low/medium/high severity remap)
...
- **Enhanced Automated Sprint**: A Claude Code skill (`/enhanced-automated-sprint`) that automates the full dev lifecycle: story creation, refinement, validation, TDD tests (unit + optional E2E), implementation, consolidated code review (BMAD 6.2 runs adversarial, edge case, and acceptance reviews internally), fixes, traceability, and sprint status updates. **6.4.0 introduces unattended-by-default execution with anti-leak commit messages, per-story auto-commit (incl. submodule commits), and a deferred-decisions log artifact** — every former pause point auto-resolves with best-judgment and writes to `{implementation_artifacts}/sprint-epic-${EPIC_ID}-deferred-decisions.md`; ambiguous merge conflicts are the only hard pause. **6.8.0** keeps the pipeline interface identical (no skill renames 6.6→6.8) but fixes the support-file paths to resolve under `.claude/skills/` via a Phase-0 `${BMAD_SKILLS_ROOT}` cascade, and constrains Step 7 code-review scope to the story File List to neutralize the new `baseline_commit` (#2403) cross-story over-scoping. **6.10.0** is also interface-identical (verified by diffing BMAD-METHOD tags v6.8.0..v6.10.0 — all nine invoked skills, the `.claude/skills` target, the three explicit-path support files, `config.js`, `bmad-dev-story`, and code-review's `step-01-gather-context.md` are all unchanged): it grants Steps 1/7 standing subagent authorization so BMAD 6.10's new "ask once now" permission line cannot stall a nested Task agent, remaps severity to BMAD's `low`/`medium`/`high` (#2523 removed the `critical` band, leaving Step 8's opus escalation dead — now re-derived from blast-radius + redesign-scope), documents the retrospective-owned `action_items:` key in `sprint-status.yaml` (6.9) as preserve-don't-touch, and flags the Step 7 token baseline break (#2524/#2525).
