...
description: 'Adds structured interaction enforcement to a default BMAD 6.0.3 Claude Code install — forces AskUserQuestion tool usage'
---

# Hotfix: Add AskUserQuestion enforcement to BMAD

You are applying a hotfix to a default BMAD 6.0.3 installation for Claude Code. This install has NO interaction_style feature. You are ADDING it from scratch.
...
3. Adds a RULE to every `.claude/commands/` entry point enforcing `AskUserQuestion`
4. Adds a mandate to `workflow.xml`
...
### Step 3 — Add RULE to every `.claude/commands/` file

For EVERY `.md` file in `.claude/commands/`, add the following line immediately after the closing `---` of the frontmatter (before any other content):
...
Example — a command file currently looks like:
```
---
name: 'create-prd'
description: '...'
...
### Step 4 — Add mandate to `_bmad/core/tasks/workflow.xml`
