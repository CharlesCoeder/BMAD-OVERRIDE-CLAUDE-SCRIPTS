...
description: 'Run the full BMAD pipeline (BMAD 6.6-targeted) for multiple stories in an epic. Breaks stories into manageable tasks, tracks progress with TaskCreate/TaskUpdate, and uses focused agents for context efficiency. Supports parallel story execution. Unattended-by-default with anti-leak commit messages, auto-commit (incl. submodules), and a deferred-decisions log.'
---

<!-- BMAD 6.4.0 → 6.6.0 audit (do not delete)
- 6.5.0: only adds new agent platforms; no skill-interface or path changes affecting this pipeline.
- 6.6.0 (#2348): `project_name` moved from `_bmad/bmm/config.yaml` to `_bmad/core/config.yaml` (auto-migrated on upgrade). All other variables read by this skill are unchanged in location.
