...
description: 'Adds structured interaction enforcement to a BMAD 6.10.0 Claude Code install — forces AskUserQuestion tool usage'
---

# Hotfix: Add AskUserQuestion enforcement to BMAD

You are applying a hotfix to a BMAD 6.10.0 installation for Claude Code. This install has NO interaction_style feature. You are ADDING it from scratch.
...
**Note:** BMAD 6.10 uses `.claude/skills/` with `SKILL.md` entry points instead of `.claude/commands/*.md`. If the project still has `.claude/commands/` (legacy layout), apply the same rule to those files as well.
