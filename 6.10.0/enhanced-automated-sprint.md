...
description: 'Run the full BMAD pipeline (BMAD 6.10-targeted) for multiple stories in an epic. Breaks stories into manageable tasks, tracks progress with TaskCreate/TaskUpdate, and uses focused agents for context efficiency. Supports parallel story execution. Unattended-by-default with anti-leak commit messages, auto-commit (incl. submodules), a deferred-decisions log, and an always-on per-step token-usage log.'
---

<!-- BMAD 6.8.0 → 6.10.0 audit (do not delete)
Verified by diffing the BMAD-METHOD tags v6.8.0..v6.10.0 directly (not the changelog prose).

NO INTERFACE BREAKS. All nine skills this pipeline invokes exist at 6.10 under identical names:
/bmad-create-story, /bmad-dev-story, /bmad-code-review, /bmad-qa-generate-e2e-tests,
/bmad-sprint-status, /bmad-correct-course, /bmad-advanced-elicitation, /bmad-quick-dev, /bmad-help.

Verified UNCHANGED 6.8 → 6.10 (no action needed):
- `.claude/skills` install target — tools/installer/ide/platform-codes.yaml claude-code entry untouched
  (the +14 lines are two NEW platforms: hermes-agent, CodeWhale). ${BMAD_SKILLS_ROOT} cascade holds.
- All three explicit-path support files: bmad-advanced-elicitation/methods.csv,
  bmad-create-story/checklist.md, bmad-quick-dev/compile-epic-context.md. 6.10 adds a SECOND copy at
  bmad-dev-auto/compile-epic-context.md — harmless; the cascade keys on bmad-create-story/checklist.md,
  and Phase 0 §9.3 reads the bmad-quick-dev copy by explicit path.
- Config layout: tools/installer/core/config.js is byte-identical. The installer still generates
  `_bmad/core/config.yaml` + `_bmad/bmm/config.yaml` per module. The four-layer TOML resolver
  (config.toml / config.user.toml / custom/*) already existed at 6.8 — no migration, no Phase 0 change.
- `bmad-dev-story` ENTIRELY unchanged, and `bmad-code-review/steps/step-01-gather-context.md`
  unchanged → `baseline_commit` stamping and diff-base logic are identical to 6.8. The Step 7
  File-List scope guard added in 6.8 is still aimed at exactly the right problem. Keep it.
- Story status vocabulary unchanged (backlog → ready-for-dev → in-progress → review → done).
- methods.csv schema unchanged; +2 methods (Subtraction #43, Map Is Not the Territory #47) and a
  renumber. Step 2 auto-selects by relevance, never by index — unaffected.
- removals.txt adds only bmad-investigate and bmad-auto-setup. Nothing this pipeline uses.
- `python3` still works for resolve_config.py / resolve_customization.py. `uv run` becomes REQUIRED
  in BMAD v7 — that is the upgrade that will force real work on this skill, not 6.10.

6.10 breaking changes are all surfaces the sprint does not consume: bmad-automator → bmad-loop
(opt-in module, not installed by default), bmad-investigate retired, bmad-create-architecture →
bmad-architecture (6.9, forwarding shim until v7), bmad-create-prd → bmad-prd (6.7 shim).

FOUR CHANGES MADE IN THIS VERSION (each marked `6.10:` inline at the edit site):

1. SUBAGENT STANDING AUTHORIZATION (highest-value fix). 6.10 added this line to BOTH
   bmad-create-story/SKILL.md and bmad-code-review/SKILL.md:
     "Subagents, when the capability is available, are an important part of this workflow…
      If you need an explicit user instruction to run them, ask once now for the whole workflow run."
   Steps 1 and 7 invoke those skills INSIDE Task agents, which cannot prompt a user. Step 7 is
   already this pipeline's #1 stall source (four incidents across three measured sprints). A nested
   agent hitting "ask once now" is a new, plausible stall. Steps 1 and 7 now grant that authorization
   up front so the question is never reached.

2. SEVERITY TAXONOMY REMAP (#2523). 6.10's triage step now assigns explicit `low`/`medium`/`high`,
   EXPLICITLY DISCARDS subagent-assigned severity ("Review subagents operate under by-design
   information asymmetry"), requires reading call sites/guards before rating, and DROPS the
   "prefer conservative when uncertain" tie-breaker. There is no `Critical` band — BMAD's top level
   is `high`. Step 8's opus-escalation trigger keyed on "Critical severity" would therefore never
   fire under 6.10. Steps 7/8 and Decision Point #4 now speak BMAD's low/medium/high vocabulary and
   re-derive the escalation trigger from blast radius instead of a band name that no longer exists.
   Expect FEWER items routed to Step 8 overall — that is BMAD deliberately deflating over-rated
   unreachable findings, and it is an improvement, not a regression. Do not compensate for it.

3. sprint-status.yaml GAINED A TOP-LEVEL `action_items:` LIST (6.9). The retrospective workflow
   appends to it; /bmad-sprint-status validates it (open | in-progress | done) and surfaces open
   entries; sprint-planning preserves it on regenerate. Step 10 is a surgical status-field flip and
   already preserves it, but the instruction now says so explicitly so the coordinator does not read
   an unfamiliar key as schema drift and "fix" it. The epic close-out reconcile will start emitting
   an "Open Action Items" block — that is expected output, not an error.

4. STEP 7 TOKEN BASELINE DRIFTS UP. The standalone deletion-contract auditor was retired and folded
   into Edge Case Hunter (#2525), which also gained a named-set generalization pass at a MEASURED
   +19% tokens/run (#2524). Step 7 averages are no longer comparable to the three sprints the
   2026-07-15 token revision was tuned on. The token-usage log now says so, so the next tuning pass
   does not misread a spec change as regression.
-->
...
4. **At sprint end, append:**
   ...
   - **Trim/bulk observations** — 3–6 bullets on what this sprint's data suggests. Compare against prior sprints when available: glob `{implementation_artifacts}/sprint-epic-*-token-usage.md` (plus any older ad-hoc usage docs the project keeps under `{project_knowledge}`), and cite cross-sprint deltas (e.g. "Step 4 avg 194k → 233k").
     - **6.10 BASELINE BREAK — Step 7 only.** Step 7 averages from sprints run on BMAD ≤ 6.8 are NOT comparable to 6.10+. Upstream folded the retired standalone deletion-contract auditor into Edge Case Hunter (#2525) and added a named-set generalization pass with a MEASURED +19% tokens/run (#2524). Expect Step 7 to read higher and do not log it as a regression or "fix" it — it is a spec change, and the catch-rate gain upstream measured (50–100% on a real regression) is what is being bought. Steps 1/1.5/2/3/4/5/6/8/9 baselines are unaffected. When comparing across the 6.8→6.10 boundary, say so explicitly in the bullet.
...
> **6.10:** if the project has run a retrospective, `/bmad-sprint-status` will now emit an **"Open Action Items"** block (from sprint-status.yaml's `action_items:` key, added in 6.9) alongside the usual counts. This is expected output, not an error and not something this sprint introduced. Pass it through to the sprint summary verbatim; do not action, close, or edit those items — they belong to the retrospective workflow.
...
Task tool:
  ...
  prompt: |
    ...
    SUBAGENT AUTHORIZATION (6.10) — BMAD 6.10's bmad-create-story/SKILL.md instructs the skill to
    "ask once now for the whole workflow run" if it needs explicit permission to launch subagents.
    You are running inside a Task agent and CANNOT prompt the user, so treat this as that permission:
    you have STANDING AUTHORIZATION to launch subagents for this entire workflow run. Do NOT ask.
    Do NOT halt to request it. If you cannot launch subagents in this environment, do the work
    inline and note the degradation in your return — never stall waiting on an answer.
...
Task tool:
  ...
  prompt: |
    ...
    Run the BMAD consolidated code review for story ${SID}.
    Execute: /bmad-code-review ${SID} yolo

    SUBAGENT AUTHORIZATION (6.10) — BMAD 6.10's bmad-code-review/SKILL.md instructs the skill to
    "ask once now for the whole workflow run" if it needs explicit permission to launch subagents.
    Its three review layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor) ARE subagents, so
    this is the one place that question is most likely to fire. You are running inside a Task agent
    and CANNOT prompt the user: you have STANDING AUTHORIZATION to launch subagents for this entire
    workflow run. Do NOT ask. Do NOT halt. Launch all applicable review layers immediately.
    If subagents are genuinely unavailable, BMAD's fallback writes prompt files to
    {implementation_artifacts} and HALTs — do NOT take that path. Run the review layers inline
    yourself instead and flag the degradation in your return so the coordinator can log it.
    ...
    BMAD's code review runs 3 internal review passes:
    ...
    2. Edge Case Hunter — method-driven path enumeration for unhandled edge cases
       (6.10: also absorbs the retired standalone deletion-contract auditor, and adds a
       named-set generalization pass that catches diffs special-casing only some members
       of a fixed set — enum, status code, sentinel, flag)
    ...
    SEVERITY VOCABULARY (6.10, #2523) — report severity using BMAD 6.10's OWN three-band scale:
    `low` | `medium` | `high`. There is NO `critical` band in 6.10; `high` is the top level.
    BMAD's triage step now (a) explicitly DISCARDS severity assigned by the individual review
    subagents and re-rates centrally, (b) requires reading surrounding source — call sites and
    guards outside the diff hunk — before rating, and (c) has dropped the old "prefer conservative
    when uncertain" tie-breaker. Pass through BMAD's final triage severity verbatim. Do NOT
    re-inflate it back to a `critical` band, and do NOT substitute the reviewers' own ratings.

    Additionally, flag (separately from severity, as a boolean per finding) whether a finding sits
    on a BLAST-RADIUS surface: security, authn/authz, payments, or data integrity/loss. The
    coordinator uses this — not a severity band — to decide Step 8's model tier.

    Return to coordinator:
    - Overall verdict: PASS / PASS WITH ITEMS / FAIL
    - Findings table with source attribution:
      (# | source [blind/edge/acceptance] | severity [low/medium/high] | blast_radius [y/n] | category | finding | recommendation)
    - High-severity count (by source)
...
Task tool:
  ...
  prompt: |
    ...
    Only `high` severity items are routed here (6.10: BMAD's top band is `high` — there is no
    `critical`); `medium` and `low` are deferred to the decisions log per the unattended defaults.

    Items to fix:
    ${CONSOLIDATED_ACTION_ITEMS}

    Edge case findings (JSON — add guards for the high-severity ones):
    ${EDGE_CASE_FINDINGS}
...
**Model escalation rule (Step 8) — RE-DERIVED FOR 6.10:** default `sonnet` — measured sprints showed it handles routine high-severity fix batches (localized guards, AC-gap patches, teardown races) with no quality bounce-back. Spawn with `model: opus` instead when ANY routed item is:
...
- on a blast-radius surface — security, authn/authz, payments, or data integrity/loss (Step 7 now returns this as an explicit per-finding `blast_radius` boolean).

> **6.10 change (#2523):** the old first trigger read "Critical severity". BMAD 6.10's triage emits only `low`/`medium`/`high` — no `critical` band — so that trigger was dead code and the escalation would never have fired. The two surviving triggers are *blast-radius* and *scope-of-fix* judgments, which is what the trigger was actually proxying for all along, and both are still observable at 6.10. Do NOT re-add a severity-band trigger; `high` alone is now too broad to escalate on (it is the routing threshold, not an escalation signal).
>
> **Expect fewer items to reach Step 8 at 6.10.** Triage now reads call sites and guards before rating, and BMAD dropped its "prefer conservative when uncertain" tie-breaker — both deliberately deflate over-rated unreachable findings. A lighter Step 8 is the intended outcome. Do NOT compensate by widening the routing threshold to `medium`.

Log the escalation choice (and which trigger fired) to `${DEFERRED_DECISIONS_PATH}` with `confidence: high, needs_human_review: no`.

**Coordinator note:** Save worktree branch/path from result for Step 9. If Step 8 was skipped (no `high` action items from Step 7), also skip Step 9.
...
2. Edit {implementation_artifacts}/sprint-status.yaml directly: flip story ${SID}'s status
   ...
   SURGICAL EDIT ONLY — never rewrite or regenerate the file.
3. Grep-verify the edit landed (`grep -A2 "${SID}" sprint-status.yaml`).
4. Mark the Step 10 task completed.

**6.10 — `action_items:` is a legitimate key, leave it alone.** BMAD 6.9 added an optional top-level
`action_items:` list to sprint-status.yaml. The retrospective workflow appends to it, /bmad-sprint-status
validates it (`open` | `in-progress` | `done`) and surfaces open entries, and sprint-planning preserves
it on regenerate. This pipeline never writes it. Because the Step 10 edit above is a surgical
status-field flip, the key survives untouched — which is correct. Do NOT treat an unfamiliar
`action_items:` block as schema drift, do NOT "normalize" or strip it, and do NOT let it fail a
status-vocabulary check (its values are a DIFFERENT enum from story/epic/retrospective statuses).
...
4. **Step 7 (consolidated review) action items:** Auto-route by severity, using BMAD 6.10's three-band scale (`low` | `medium` | `high` — there is no `critical` band; see the 6.8→6.10 audit). `high` → straight to Step 8 for fix. `medium` and `low` → append to `${DEFERRED_DECISIONS_PATH}` with `needs_human_review: yes` and skip the fix. If only `medium`/`low` items exist (no `high`): mark Steps 8 AND 9 completed immediately and proceed to Step 10. (The user-presentation block from older versions is gone.) A clean or near-clean Step 7 is a MORE likely outcome at 6.10 than in the measured sprints — #2523 deliberately deflates over-rated findings. Do not widen the threshold to `medium` to compensate, and do not re-run the review to "find more".
