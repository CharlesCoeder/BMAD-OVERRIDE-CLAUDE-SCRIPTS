...
- **Model assignments rationalized from the data.** The measured sprints deliberately ran an intelligence-beefed config (Opus on most steps, strongest available model as coordinator); the usage data now settles per-step allocation. Step 5 (implement) standardized on **Opus** — every measured run used it, and implementation quality is the core deliverable. Step 8 (fixes) is **severity-adaptive**: Sonnet by default (it handled routine Critical/High fix batches with no bounce-back), escalated to Opus for Critical, redesign-scope, or security/payment-surface items. Steps 1/2/7 stay Opus (authoring compounds downstream; elicitation and review produced the highest-value catches). Step 6 stays Opus but only spawns in parallel mode. A coordinator-model note recommends running sprints from the most capable session tier and re-mapping model fields by role when the lineup shifts.
